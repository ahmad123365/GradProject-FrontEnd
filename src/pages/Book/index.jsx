...
const Book = () => {
	...
	console.log(book)
	return (
		<div className='w-[95%] md:w-[70%] lg:w-[65%] mx-auto mt-9'>
				<div className='w-full flex gap-[2rem] flex-col md:flex-row'>
					<div className='mx-auto md:mx-0  w-[30%] flex flex-col items-center'>
					<img alt='' src={book?.imageCover} className='w-full px-[3%] h-[16rem] rounded-lg ' />
					<button className={`w-[90%] bg-[#00A9FF] text-[#fff] mt-3 py-1 px-2 rounded-xl text-[.8rem] font-semibold hover:text-[#00A9FF] hover:bg-[#eee] transition-colors duration-300`}>
						Add to Cart
					</button>
					...
					<div className='w-full md:w-[70%]'>
						...
						<h1 className=' text-[1rem]   opacity-70 '><span className='text-[.6rem] text-[#3C4856]'>by</span> {book?.author}</h1>
						...
						<p className='text-[.8rem] mt-4 text-justify mb-6'>{book?.description}</p>
						<h1 className=' text-[1rem] '>Publish Year: {book?.publishYear}</h1>
						<h1 className=' text-[1rem] '>Genre: {book?.categories[0]}</h1>
					</div>
				</div>
				<div>
