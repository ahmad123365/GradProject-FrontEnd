...
export const getAllBooks = () => dispatch => {
	api.get("/api/v1/books", tokenConfig())
	.then (res => {
		dispatch({
...
export const getBook = (id) => dispatch => {
	api.get(`/api/v1/books/${id}`, tokenConfig())
	.then (res => {
		dispatch({
...
export const createBook = (body) => dispatch => {
	api.post("/api/v1/books", body, tokenConfig())
	.then (res => {
		dispatch({
...
export const updateBook = (id, body) => dispatch => {
	api.patch(`/api/v1/books/${id}`, body, tokenConfig())
	.then (res => {
		dispatch({
...
export const deleteBook = (id) => dispatch => {
	api.delete(`/api/v1/books/${id}`, tokenConfig())
	.then (res => {
		dispatch({
...
export const buyBook = (body) => dispatch => {
	api.post(`/api/v1/books/buy`, body, tokenConfig())
